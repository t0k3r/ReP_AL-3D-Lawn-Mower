...
int  Home_Wire_Compass_Heading      = 100;                    // Heading the Mower will search for the wire once the mowing is completed.
int  CPower                         = 200;                      // Magnification of heading to PWM - How strong the mower corrects itself in Compass Mowing
...
float Battery_Min               = 12.8;                       // Lower Limit of battery charge before re-charge required.
...
long maxdistancesonar           = 9;                         // distance in cm from the mower that the sonar will activate at.

//Wheel Motors Setup
int Max_Cycles                 = 200;                         // Number of loops the Sketch will run before the mower just turns around anyway.
int PWM_MaxSpeed_LH            = 250;     //EEPROM            // Straight line speed LH Wheel (Looking from back of mower)  Will be overidden if saved in EEPROM
int PWM_MaxSpeed_RH            = 245;     //EEPROM            // Straight line speed RH Wheel - adjust to keep mower tracking straight.  Will be overridden if saved in EEPROM
...
int Mower_Turn_Delay_Max        = 3000;                       // A random turn time between these numbers is selected by the software
int Mower_Reverse_Delay         = 3000;                       // Time the mower revreses at the wire
...
*/
  ...
  // Wire detection Values
    ...
    int OutMid = 400;
    int OutMax = 1500;
                                    // the maximum received signal value outside the wire
